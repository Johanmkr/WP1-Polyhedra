...
def get_polytope_list(
    first_sample_bit_vector,
    indexes_for_active_bits_first_sample,
    input_dimension=2,
    model=None,
):
    polytope_list = []
    ...
    for index in indexes_for_active_bits_first_sample:
        ...
        if any(
            np.array_equal(copy_of_first_sample_bit_vector, polytope)
    ...
    while i < len(polytope_list):
        ...
        A, c = get_inequalities(
            model,
            [split_vectors],
            is_input_sample=False,
            return_bit_vec_list=False,
        ...
        for index in indexes_for_active_bits:
            ...
            if any(
                np.array_equal(copy_of_bit_string, polytope)
                for polytope in polytope_list
            ):
                continue
            else:
...
def bit_vector_from_relus(
    model,
    input_vectors,
    stack_layers=False,
    verbose=False,
    get_unique_bit_vectors_only=True,
):
    ...
    for i in range(input_vectors.shape[1]):
        ...
        if get_unique_bit_vectors_only:
            if any(
                np.array_equal(bit_list_for_a_vector, vector)
                for vector in all_bit_vector_list
            ):
                continue
...
def get_inequalities(
    model,
    test,
    is_input_sample=True,
    verbose=False,
    return_bit_vec_list=True,
    ...
    if return_bit_vec_list:
        ...
        stacked_bit_vector_list = [
            np.concatenate(element, axis=0) for element in bit_vector_list_of_samples
        ]
...
def clip_vertices(vertices, bound):
    ...
    for i in range(len(vertices)):
        ...
        next_vertex = vertices[
            (i + 1) % len(vertices)
        ]  # Get the next vertex (wraps around to the first vertex)
